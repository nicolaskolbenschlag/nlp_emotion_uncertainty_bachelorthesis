...
from modules.multimodal_transformer import MULTModel
import utils
...
class QuantileRegressor(nn.Module):
    def __init__(self, d_in, d_hidden, d_out, dropout=0, bias=0):
        ...
        self.fc_1 = nn.Sequential(nn.Linear(d_in, d_hidden), nn.ReLU(True), nn.Dropout(dropout))
        self.fc_2 = nn.Linear(d_hidden, 3)#d_out)# TODO d_out * 3
        nn.init.constant_(self.fc_2.bias.data, bias)
...
class Model(nn.Module):
    def __init__(self, params):
        ...
        if params.transformer:
            feature_dims = params.feature_dims
            dims = {'dim_l': feature_dims[0], 'dim_a': feature_dims[1], 'dim_v': feature_dims[2], 'dim_out': d_rnn_in,
                    'dim_window': params.win_len}
            self.mmt = MULTModel(dims)
    ...
    def forward(self, x, x_len):
        ...
        if self.params.transformer:
            ...
            f_a = x[:, :, self.params.feature_dims[0]: self.params.feature_dims[0] + self.params.feature_dims[1]]
            f_v = x[:, :, self.params.feature_dims[0] + self.params.feature_dims[1]:]
...
class FusionModel(nn.Module):
    def __init__(self, params):
        ...
        self.params = params
        if params.attn:
            self.attn = SelfAttention(params.n_layers, params.d_in, params.n_heads, dropout=params.attn_dr)
